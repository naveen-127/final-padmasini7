	...
	// Add this method for moving special subject lessons
	 public boolean moveSpecialSubjectLesson(WrapperUnit data, String direction) {
	     ...
	     System.out.println("📊 Data: db=" + data.getDbname() + ", subject=" + data.getSubjectName());
	     
	     // For special subjects, lessons are at the root level
	     MongoTemplate mongoTemplate = getTemplate(data.getDbname());
	     String collectionName = data.getSubjectName();
	     
	     // Get all units
	     Query query = new Query();
	     List<UnitRequest> allUnits = mongoTemplate.find(query, UnitRequest.class, collectionName);
	     ...
	     // Find the target unit and its index
	     int targetIndex = -1;
	     for (int i = 0; i < allUnits.size(); i++) {
	         if (allUnits.get(i).getId().equals(data.getParentId())) {
	             targetIndex = i;
	             break;
	         }
	     }
	     ...
	     // Perform the move
	     if ("up".equals(direction) && targetIndex > 0) {
	         // Swap with the previous unit
	         UnitRequest temp = allUnits.get(targetIndex - 1);
	         allUnits.set(targetIndex - 1, allUnits.get(targetIndex));
	         allUnits.set(targetIndex, temp);
	         System.out.println("⬆️ Moved up from " + targetIndex + " to " + (targetIndex - 1));
	     } else if ("down".equals(direction) && targetIndex < allUnits.size() - 1) {
	         // Swap with the next unit
	         UnitRequest temp = allUnits.get(targetIndex + 1);
	         allUnits.set(targetIndex + 1, allUnits.get(targetIndex));
	         allUnits.set(targetIndex, temp);
	         System.out.println("⬇️ Moved down from " + targetIndex + " to " + (targetIndex + 1));
	     } else {
	         System.out.println("⚠️ Cannot move " + direction + " - at boundary");
	         return false;
	     }
	     
	     // Delete all and reinsert in new order
	     mongoTemplate.remove(query, UnitRequest.class, collectionName);
	     mongoTemplate.insert(allUnits, collectionName);
	     
	     System.out.println("✅ Special subject lesson moved successfully");
	     return true;
	 }
	 
	// Add this method for moving root-level units
	 public boolean moveRootUnit(String dbName, String subjectName, String unitId, String standard, String direction) {
	     System.out.println("🔄 Moving root unit ID: " + unitId + " direction: " + direction);
	     
	     MongoTemplate mongoTemplate = getTemplate(dbName);
	     
	     // Define subjects that don't require standard
	     List<String> subjectsWithoutStandard = Arrays.asList("NEET Previous Questions", "Formulas", "JEE Previous Questions", "Previous Questions");
	     
	     // Create query based on whether standard is needed
	     Query query;
	     if (subjectsWithoutStandard.contains(subjectName)) {
	         query = new Query();
	     } else {
	         query = new Query(Criteria.where("standard").is(standard));
	     }
	     
	     // Get all root units with sorting
	     List<UnitRequest> allUnits = mongoTemplate.find(query, UnitRequest.class, subjectName);
	     ...
	     System.out.println("📊 Found " + allUnits.size() + " root units");
	     
	     // Find the target unit
	     int targetIndex = -1;
	     for (int i = 0; i < allUnits.size(); i++) {
	         if (allUnits.get(i).getId().equals(unitId)) {
	             targetIndex = i;
	             System.out.println("🎯 Target found at index: " + i + " - " + allUnits.get(i).getUnitName());
	             break;
	         }
	     }
	     ...
	     if ("up".equals(direction) && targetIndex > 0) {
	         ...
	         System.out.println("⬆️ Moving up from " + targetIndex + " to " + (targetIndex - 1));
	         UnitRequest temp = allUnits.get(targetIndex - 1);
	         allUnits.set(targetIndex - 1, allUnits.get(targetIndex));
	         allUnits.set(targetIndex, temp);
	     } else if ("down".equals(direction) && targetIndex < allUnits.size() - 1) {
	         // Swap with next unit
	         System.out.println("⬇️ Moving down from " + targetIndex + " to " + (targetIndex + 1));
	         UnitRequest temp = allUnits.get(targetIndex + 1);
	         allUnits.set(targetIndex + 1, allUnits.get(targetIndex));
	         allUnits.set(targetIndex, temp);
	     } else {
	     ...
	     // Delete all and reinsert in new order
	     // Note: This approach maintains the order. If you have a lot of data, 
	     // you might want a more efficient approach, but this works for now
	     mongoTemplate.remove(query, UnitRequest.class, subjectName);
	     
	     for (UnitRequest unit : allUnits) {
	         mongoTemplate.save(unit, subjectName);
	     }
	     
	     System.out.println("✅ Root unit moved successfully");
	     return true;
	 }
